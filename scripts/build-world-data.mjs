...
async function fetchBytes(url) {
  const response = await fetch(url, { signal: AbortSignal.timeout(180_000), headers: { 'User-Agent': 'Orten-2.0-build/1.0' } });
  if (!response.ok) throw new Error(`${url} svarade ${response.status}`);
  return Buffer.from(await response.arrayBuffer());
}
async function fetchText(url) {
  const response = await fetch(url, { signal: AbortSignal.timeout(60_000), headers: { 'User-Agent': 'Orten-2.0-build/1.0' } });
  if (!response.ok) throw new Error(`${url} svarade ${response.status}`);
...
try {
  console.log('Hämtar GeoNames cities500…');
